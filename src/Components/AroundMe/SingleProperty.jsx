...
const image = property.propertyImage.image.url;
const parentDiv = css `
height:357px;
margin:5px 10px;
  ...
  return (
    ...
    <Collapse in={mouseIn} collapsedSize={70} orientation="vertical" timeout={500} css={contentContainer}>
                <div css={priceDiv}>{/*  Price section */}
                    <p> {property.price?.options[0]?.formattedDisplayPrice || "$$"} <span> / {property.price.priceMessages[0].value}</span></p> 
                </div>
                <div css={nameDiv}>{/*  Name section */}
                    <h3>{property.name}</h3>
                </div>
                    <a href={`/Amenity/property/${property.id}`}>
